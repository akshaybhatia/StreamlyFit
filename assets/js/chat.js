	  ...
	  success: function(data) {
		$.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
				...
				$.post(site_url+"chat/update_message_state", { id: id} , function(data){	
...
function checkChatBoxInputKey(event,chatboxtextarea,chatuser_id,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			...
			$.post(site_url+"chat/sendchat", {to_id: chatuser_id,to: chatboxtitle, message: message} , function(data){
